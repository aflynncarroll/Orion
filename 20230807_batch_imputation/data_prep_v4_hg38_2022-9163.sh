...
##### inputs
file_loc="/geschwindlabshares/HumanGenotypeArray/G-ACE.Illumina.GSA/2022-9163\ Geschwind\ GSAMD/plinkable.HM.fwd"
#file_loc=$1
...
##### create directories
mkdir ${workdir}
mkdir ${workdir}/temp_plink
mkdir ${workdir}/temp_vcf
...
plink2 \
    ...
    --genotyping-rate \
    --make-bed \
    --out "${workdir}/temp_plink/${bfile_name}"

##### make vcf from plink bed/bim/fam for crossmap temp to hg38
plink \
    --bfile "${workdir}/temp_plink/${bfile_name}"  \
    --keep-allele-order \
    --snps-only 'just-acgt' \
    --recode vcf-iid bgz \
    --out "${workdir}/temp_vcf/${bfile_name}.temp"


###### recode sex chromosome: 23 -> X
temp_vcf="${workdir}/temp_vcf/${bfile_name}.temp.vcf.gz"
temp_vcf_x="${workdir}/temp_vcf/${bfile_name}.temp_x.vcf.gz"

zcat $temp_vcf | awk '{ if ($0 ~ /^#/ || $0 ~ /^23\t/) { sub(/^23\t/, "X\t", $0); print } else { print } }' | bgzip > ${temp_vcf_x}

##### convert temp to hg38
...
hg38_vcf="${temp_vcf_x}"
# since I did not do crossmap - i just set the temp vcf as the hg38_vcf
