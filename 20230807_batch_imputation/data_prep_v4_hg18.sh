...
##### create directories
mkdir ${workdir}
mkdir ${workdir}/hg18_plink
mkdir ${workdir}/hg18_vcf
...
plink2 \
    ...
    --genotyping-rate \
    --make-bed \
    --out "${workdir}/hg18_plink/${bfile_name}"

##### make vcf from plink bed/bim/fam for crossmap hg18 to hg38
plink \
    --bfile "${workdir}/hg18_plink/${bfile_name}"  \
    --keep-allele-order \
    --snps-only 'just-acgt' \
    --recode vcf-iid bgz \
    --out "${workdir}/hg18_vcf/${bfile_name}.hg18"


###### recode sex chromosome: 23 -> X
hg18_vcf="${workdir}/hg18_vcf/${bfile_name}.hg18.vcf.gz"
hg18_vcf_x="${workdir}/hg18_vcf/${bfile_name}.hg18_x.vcf.gz"

zcat $hg18_vcf | awk '{ if ($0 ~ /^#/ || $0 ~ /^23\t/) { sub(/^23\t/, "X\t", $0); print } else { print } }' | bgzip > ${hg18_vcf_x}
